...
[Stages 11–13] Hybrid Score Fusion (Semantic 30% + Tech Fit 30% + BM25 20% + Behavioral 20%)
       │
       ▼
[Optional Stage 2] Cross-Encoder Precision Reranking (Top 500 Pool)
       │
       ▼
...
* **Optional Stage 2 (Cross-Encoder Reranking):** If enabled (`USE_CROSS_ENCODER=true`), the top 500 candidates undergo deep pairwise evaluation against the exact job description text using `ms-marco-MiniLM-L6-v2`, blending cross-encoder logits with Stage 1 scores for ultimate ranking precision.
...
**Passing Custom Flags in Docker:**
```bash
docker run --rm \
  -v /path/to/local/data:/app/data \
  -v /path/to/local/output:/app/output \
  notbad007/redrob-ranker:latest --candidates /app/data/candidates.jsonl --out /app/output/submission.csv --use-cross-encoder true
```
