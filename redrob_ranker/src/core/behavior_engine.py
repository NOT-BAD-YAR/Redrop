...
def _active_days_multiplier(sig: Dict[str, Any]) -> float:
    cfg = WEIGHTS.get("active_days_penalty", {})
    if not cfg:
        return 1.0
    ...
    threshold_moderate = int(cfg.get("threshold_moderate", 90))
    threshold_severe = int(cfg.get("threshold_severe", 180))
    penalty_moderate = float(cfg.get("penalty_moderate", 0.10))
    penalty_severe = float(cfg.get("penalty_severe", 0.20))

    if days_ago > threshold_severe:
        return 1.0 - penalty_severe
    if days_ago > threshold_moderate:
        return 1.0 - penalty_moderate
    return 1.0
...
def calculate_behavior(cand: Dict[str, Any], trace: Dict[str, Any]) -> None:
    ...
    min_mult = WEIGHTS.get("min_multiplier", 0.55)
    ...
    trace["behavior"]["active_days_multiplier"] = _active_days_multiplier(sig)
